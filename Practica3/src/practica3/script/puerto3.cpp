...
bool tormentaAfecta(){
  return !(tormenta == false || (tormenta == true && remolcador_antitormenta == true));
}
...
void llegada_barco()
{
  ...
  if ((tormentaAfecta() == false) && (remolcador == LIBRE) && (atraques_libres > 0))
...
void fin_carga()
{
  ...
  acum_at_cargando += (reloj-tdus_at)*(num_atraques-atraques_libres-encola_sal);
  tdus_at = reloj;
  acum_carga += cargas[nodo.reg_cola.tipo];
  if ((remolcador == LIBRE) && (tormentaAfecta() == false)) //comienza a desatracar el barco
     {
     acum_rem_amarrado += reloj-tdus_rem;
     tdus_rem = reloj;
     remolcador = OCUPADO;
...
void fin_desatraque()
{
  ...
  if ((encola_lleg > 0) && (tormentaAfecta() == false)) //comienza a atracar un barco
     {
     acum_lleg += (reloj-tdus_lleg)*encola_lleg;
     tdus_lleg = reloj;
     encola_lleg --;
...
void fin_simulacion()
{
  ...
  informe[cont_simu][11] = acum_carga;
}
...
void generador_informes(int simulaciones)
{
  ...
  printf("\nMedia de toneladas cargadas: media(%f), dt(%f)\n\n",media[11],dt[11]);*/
}
...
int main(int argc, char *argv[])
{
  ...
  }else
  {
    ...
    printf("\nLos parametros con * no son obligatorios.");
